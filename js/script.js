...
function renderGallery() {
  ...
  items.forEach((item) => {
    ...
    downloadBtn.addEventListener("click", (e) => {
      downloadImage(e, fullSizeSrc, item.alt || "wallpaper");
    });
...
function getFullSizeUrl(url) {
  if (url.includes("raw.githubusercontent.com")) {
    if (url.includes("/ios/blocks/")) {
      return url.replace("/blocks/", "/").replace(".webp", ".tendies");
    }
    if (url.includes("/wp/cards/")) {
      return url.replace("/cards/", "/").replace(".webp", ".zip");
    }
...
function downloadImage(e, url, filename) {
